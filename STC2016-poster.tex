...
% 		  \includegraphics[width=.4\colwidth]{abb/TTM_1pulse.eps}
      ...
      \column{0.58}{
		\block{Models and Methods}{
		  ...
		  \innerblock{Two-Temperature Model (2TM)}{
			\begin{itemize}
			  \item calculates $T_\mathrm{el}$ and $T_\mathrm{ph}$ from material constants and laser parameters e.g.:
			  \begin{itemize}
				\item penetration depth (dependent on laser wavelength and material)
				\item pulse duration
				\item effective absorbed fluence (energy/area)  
			  \end{itemize}
			\end{itemize}
		  }
		  \vspace{.5cm}
