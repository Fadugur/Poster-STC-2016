		...
		\block{Models and Methods}{
		  \innerblock{Six-dimensional Potential Energy Surface (6D PES)}{
		    \begin{minipage}[t]{.66\colwidth}
		      \begin{itemize}
				\item Basis for dynamics: precomputed Potential Energy Surface (PES)
		      \end{itemize}
		      \begin{adjustbox}{valign=t}
		  ...
		  \innerblock{Two-Temperature Model (2TM) \cite{anisimov:74}}{
			\begin{minipage}[t]{.5\colwidth}
			  \begin{itemize}
				\item consists of two coupled differential equations 
				\item calculates $T_\mathrm{el}$ and $T_\mathrm{ph}$ from laser parameters, e.g.: 
				{\small
				\begin{itemize}
				  \item pulse duration
				  \item effective absorbed fluence $F$ (energy/area)				
				  \item laser wavelength (affects penetretion depth into material)
				\end{itemize}				
				}
				\item and from material properties
			  \end{itemize}	
			\end{minipage}
			\begin{adjustbox}{valign=t}
			  \begin{minipage}[t]{.5\colwidth}
% 				\includegraphics[width=.15\colwidth]{abb/Part1SurfScheme.png}\\[.3\baselineskip]
				\includegraphics[width=.4\colwidth]{abb/2TM_equs.png}
		      \end{minipage}
			\end{adjustbox}
